...
import com.lyj.config.interceptor.CrossDomainInterceptor;
...
public class WebConfig implements WebMvcConfigurer {
    ...
    @Autowired
    CrossDomainInterceptor crossDomainInterceptor;//登入拦截器
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        InterceptorRegistration crossDomainRegistration = registry.addInterceptor(crossDomainInterceptor);//添加跨域拦截器
        crossDomainRegistration.addPathPatterns("/**");//拦截所有请求
        crossDomainRegistration.order(1);//设置拦截器的优先级

        //这个拦截的要看的就是请求的url是否包含指定的内容
        InterceptorRegistration loginCheckInterceptor = registry.addInterceptor(loginInterceptor);//添加登入拦截器
        loginCheckInterceptor.order(2);
